...
from fullyconection import tryFC
import random
...
train_data = r'traindata.npy'
train_label=r'trainlabel.npy'
valid_data=r'developdata.npy'
valid_label=r'developlabel.npy'
batch_size=16
STEP_SIZE = 10  # 1
LR_DECAY = 0.5  # 0.7
#load data
...
model = Vggish(3).to(device)
# model = nn.DataParallel(model).to(device)
WD =1e-5
criterion= torch.nn.CrossEntropyLoss()
optimizer=torch.optim.Adam(model.parameters(),lr=1e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=WD)
accuracy=0
...
def train(epoch):
    ...
    running_loss = running_loss /len(datasettrain.dataset)
...
def val(epoch):
    ...
    loss = loss / len(datasetvalid.dataset)
    actu_all,pred_all = vote_data(pred_all,actu_all,outputitem)
    test_result=metrics.accuracy_score(actu_all, pred_all)
    cl_result=metrics.recall_score(actu_all, pred_all,average='macro')
    loss = loss.cpu().numpy().copy()
    print('Accuracy on test:  ',str(test_result),' runningloss:',str(loss))
    print('classical accuracy',str(cl_result))
    pred_all = np.array(pred_all,dtype=int)
    actu_all = np.array(actu_all,dtype=int)
    print('actu:',actu_all)
    print('pred:',pred_all)
