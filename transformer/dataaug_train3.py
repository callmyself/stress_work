...
from dataaug_model7 import Transformers
import numpy as np
import pandas as pd
...
def hsf_data(data,callen,interval):
    '''
    产生hsf特征，对callen token 做 max,min,mean,std,med统计,overlap为interval token长度
    '''
    finallen = 500//(callen-interval) -1#最终特征时间长度
    lens = data.shape[0]
    feas = data.shape[1]
...
def gen_data(csv,fea_path,feacsv,fealen):
    ...
    for i in range(len(name)):
        fea_name = name[i]
        temp = np.array(pd.read_csv(fea_path+fea_name[:-4]+'_'+feacsv+'.csv',sep=';').iloc[:,2:])
...
def gen_features(texts,textnet,tokenizer):
    ...
    print(input_masks_tensors.shape)
...
# fea_path2 = r'../../../../cnn/folder1/feature/5s_lld_avec20132/' 
# text_path = r'../../../../../data_asr_5s_1/'
# fea_path = r'../../../../cnn/folder1/feature/5s_wav2vec1/'
...
# del train_audio_data1,train_label,develop_audio_data1,develop_label
...
# del train_audio_data2,develop_audio_data2
...
EPOCH = 60
STEP_SIZE=10
optimizer = torch.optim.Adam(model.parameters(), lr=2e-5, betas=(0.9, 0.999), eps=1e-08, weight_decay=WD)
