...
from transformer_model9 import Transformers
import numpy as np
import pandas as pd
...
def hsf_data(data,callen,interval):
    finallen = 1000//(callen-interval) -1
    lens = data.shape[0]
    feas = data.shape[1]
...
def gen_data(csv,fea_path,feacsv,fealen):
    ...
    feas = np.zeros((len(name),99,fealen*5))
...
def gen_data2(csv,fea_path):
    label = np.array(pd.read_csv(csv).iloc[:,1])
    name = np.array(pd.read_csv(csv).iloc[:,0])
    feas = np.zeros((len(name),499,512))
    for i in range(len(name)):
        fea_name = name[i]
        fea = np.load(fea_path+fea_name[:-4]+'.npy')
        newfea = np.zeros((499,512))
...
'''
'''
# traincsv = r'./newtrain4.csv'
# developcsv = r'./newdevelop4.csv'
# fea_path2 = r'../../../../cnn/folder1/feature/lld_avec2013/' 
# text_path = r'../../../../../data_asr/'
# fea_path = r'../../../../cnn/folder1/feature/wav2vec/'
